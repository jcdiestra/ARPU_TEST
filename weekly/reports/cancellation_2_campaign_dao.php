...
$sql="
select t1.week_date as 'Week Date',round(1+((datediff(t1.week_date,init))/7),0) 'Week #'
,t3.day_users as 'Total users',
...
from cancellations
left join campaign
on cancellations.campaignid=campaign.campaignid
where cancellations.campaignid=".$campaign_id."
...
left join campaign
on cancellations.campaignid=campaign.campaignid
where cancellations.campaignid=".$campaign_id_2."
group by STR_TO_DATE(concat(yearweek(date,1),' monday'),'%X%V %W')) as t1
left join
(select week_date,sum(day_users) as day_users
from users_weekly
